...
const submitWorkflowLocal = async (proj, projectConf, inputsize) => {
...
const getJobMetadataLocal = async (proj) => {
...
const updateJobStatusLocal = async (job, proj) => {
  // get job status
  const projHome = `${config.IO.PROJECT_BASE_DIR}/${proj.code}`;
...
const abortJobSlurm = async (proj, job) => {
  // To stop the running pipeline depends on the executor.
  // If is local, find pid in .nextflow.pid and kill process and all descendant processes: pkill -TERM -P <pid>
  // If is slurm, delete slurm job?
  // get slurm jobId
...
const submitWorkflow = async (proj, projectConf, inputsize) => {
  if (config.NEXTFLOW.EXECUTOR === 'local') {
    await submitWorkflowLocal(proj, projectConf, inputsize);
  } else if (config.NEXTFLOW.EXECUTOR === 'slurm') {
    await submitWorkflowSlurm(proj, projectConf, inputsize);
  } else {
...
const generateRunStats = async (project) => {
  let stats = [];
  if (config.NEXTFLOW.EXECUTOR === 'local') {
    stats = await getJobMetadataLocal(project);
  } else if (config.NEXTFLOW.EXECUTOR === 'slurm') {
    stats = await getJobMetadataSlurm(project);
  } else {
    throw Error(`Unsupported nextflow executor '${config.NEXTFLOW.EXECUTOR}'`);
  }
...
const updateJobStatus = async (job, proj) => {
  if (config.NEXTFLOW.EXECUTOR === 'local') {
    await updateJobStatusLocal(job, proj);
  } else if (config.NEXTFLOW.EXECUTOR === 'slurm') {
    await updateJobStatusSlurm(job, proj);
  } else {
    throw Error(`Unsupported nextflow executor '${config.NEXTFLOW.EXECUTOR}'`);
  }
};
