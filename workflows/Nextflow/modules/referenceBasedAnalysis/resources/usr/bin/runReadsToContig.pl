...
if ($file_long)
{
   ...
   &executeCommand("samclip --ref $ref_file --max $max_clip  < $outDir/LongReads$$.sam | samtools view -t $ref_file.fai -uhS - | samtools sort -T $tmp -@ $samtools_threads -O BAM -o $outDir/LongReads$$.bam - ") if ( -s "$outDir/LongReads$$.sam");
}
if ($paired_files){
   ...
   &executeCommand("samclip --ref $ref_file --max $max_clip  < $outDir/paired$$.sam | samtools sort -n -T $tmp -l 0 -@ $samtools_threads |  samtools fixmate -m -@ $samtools_threads  - - | samtools sort -T $tmp -@ $samtools_threads -O BAM -o $outDir/paired$$.bam - ") if (-s "$outDir/paired$$.sam");
}
...
if ($singleton)  
{
    ...
    &executeCommand("samclip --ref $ref_file --max $max_clip  < $outDir/singleton$$.sam  | samtools view -t $ref_file.fai -uhS $outDir/singleton$$.sam | samtools sort -T $tmp -@ $samtools_threads -O BAM -o  $outDir/singleton$$.bam - ") if  (-s "$outDir/singleton$$.sam");
} 
