...
def read_params(args):
    ...
    p.add_argument( '--nc', type=str, help = "to disable hierarchical clustering\n" )
    ...
    p.add_argument( '--cm_h', type=float, default = 0.01,
                    help="Set the height of the colormap. Default = 0.01" )
...
def read_table( fin, xstart,xstop,ystart,ystop, percentile = None, top = None, tax_lev = 's' ):
    mat = [l.strip().split('\t') for l in open( fin )]
    if tax_lev != 'a':
        i = tax_units.index(tax_lev)
        mat = [m for i,m in enumerate(mat) if i == 0 or m[0] ]
...
#     else:
#         if sample_dend_col_th == 'correlation':
...
#     Z2 = sch.dendrogram(Y2, no_plot=True, color_threshold=sample_dend_col_th)
    ...
    idx1, idx2 = range(ylen), range(xlen)
    #D = D[idx1,:][:,idx2]
